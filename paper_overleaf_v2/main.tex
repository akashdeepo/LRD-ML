...
\citep{he2025lrd, baillie2019longmemory}. Fractionally integrated models such as ARFIMA and FIGARCH provide a natural framework for capturing this behavior and remain statistically significant even after accounting for heavy-tailed innovations and nonlinear dynamics \citep{he2025lrd}. These findings suggest that the long-memory parameter summarizes fundamental aspects of volatility persistence. Importantly, recent evidence shows that the degree of persistence itself is time-varying and regime-dependent. Using ARFIMA and FIGARCH models, \citet{lahmiri2021covid} document significant shifts in estimated long-memory parameters during the COVID-19 pandemic across cryptocurrency and international equity markets, with volatility exhibiting markedly stronger persistence during periods of systemic stress.

\vspace{0.3cm}
\noindent
In related work, \citet{bollerslev2016risk} show that long-run dependence and fractional cointegration play a central role in linking risk and return over long horizons, providing further evidence that fractional parameters capture economically meaningful information rather than serving solely as statistical devices.
Related evidence further shows that long memory remains a dominant feature of volatility even when accounting for nonlinearities and multifractality, particularly during periods of extreme market stress \citep{lahmiri2023extreme}.


\vspace{0.3cm}
\noindent
In parallel, reduced-form approaches such as the Heterogeneous Autoregressive (HAR) model have been shown to approximate long-memory behavior using multi-scale volatility aggregation. While HAR models perform competitively in practice, recent evidence indicates that volatility dynamics and HAR coefficients vary across market regimes, particularly during periods of heightened uncertainty \citep{blake2025regime}. This instability highlights the importance of modeling time variation in persistence and suggests that summary measures of dependence may carry valuable regime-level information.
...
\citep{omer2026mlrv, zhang2024intraday}. These gains are especially pronounced at longer forecast horizons, where persistence and long-memory effects dominate volatility dynamics. More recently, deep learning architectures have been shown to extract multiscale dependence directly from high-frequency data. For example, \citet{moreno2024deepvol} propose DeepVol, a dilated convolutional neural network that maps raw intraday returns directly to daily volatility forecasts, demonstrating strong performance without relying on explicit parametric volatility models.

\vspace{0.3cm}
\noindent
Despite these advances, existing approaches typically learn long-range dependence implicitly through lagged inputs or model structure. The estimated memory parameter itself is rarely incorporated explicitly into the forecasting stage. This omission is notable given evidence that volatility regimes, structural changes, and higher-order dependence play a central role in predictive performance \citep{blake2025regime}. Moreover, the implicit treatment of persistence limits interpretability and may reduce robustness when volatility dynamics shift across regimes.

\vspace{0.3cm}
\noindent
Recent regime-aware and feature-engineered forecasting frameworks further demonstrate that explicitly incorporating structural information improves volatility prediction, particularly during turbulent periods \citep{blake2025regime}. Complementary evidence from \citet{rosenbaum2024universality} shows that machine learning models trained on pooled cross-asset data uncover a universal volatility formation mechanism that can be closely approximated by parsimonious rough-volatility specifications governed by a single memory or roughness parameter. This result reinforces the central structural role of persistence even in highly flexible data-driven models.
...
parameters. Our key insight is that the memory parameter captures fundamental properties of the underlying price process that may have predictive power for future volatility, independent of its role in model specification. Specifically, we extract long-memory diagnostics from ARFIMA and FIGARCH models and incorporate them directly into machine learning forecasting frameworks. By conditioning learning algorithms on economically meaningful measures of volatility persistence and regime intensity, we bridge traditional long-memory econometrics and modern machine learning, and show that explicit long-range dependence features improve volatility forecasts, particularly at longer horizons and during periods of market stress.
...
\bibitem[Lahmiri and Bekiros(2021)]{lahmiri2021covid}
Lahmiri, S., \& Bekiros, S. (2021).
The effect of COVID-19 on long memory in returns and volatility of cryptocurrency and stock markets.
\emph{Chaos, Solitons \& Fractals}, 151, 111221.
https://doi.org/10.1016/j.chaos.2021.111221

\bibitem[Moreno-Pino and Zohren(2024)]{moreno2024deepvol}
Moreno-Pino, F., \& Zohren, S. (2024).
DeepVol: Volatility forecasting from high-frequency data with dilated causal convolutions.
\emph{Quantitative Finance}, 24(8), 1105--1127.
https://doi.org/10.1080/14697688.2024.2387222

\bibitem[Rosenbaum and Zhang(2024)]{rosenbaum2024universality}
Rosenbaum, M., \& Zhang, J. (2024).
On the universality of the volatility formation process: When machine learning and rough volatility agree.
\emph{Frontiers of Mathematical Finance}, 3(1), 106--126.
https://doi.org/10.3934/fmf.2024002

\bibitem[Lahmiri and Bekiros(2023)]{lahmiri2023extreme}
Lahmiri, S., \& Bekiros, S. (2023).
Long memory, multifractality, and volatility dynamics under extreme market conditions.
\emph{Chaos, Solitons \& Fractals}, 169, 113286.
https://doi.org/10.1016/j.chaos.2023.113286

\bibitem[Bollerslev et al.(2016)]{bollerslev2016risk}
Bollerslev, T., Osterrieder, D., Sizova, N., \& Tauchen, G. (2016).
Risk and return: Long-run relationships, fractional cointegration, and return predictability.
\emph{Journal of Econometrics}, 193(2), 519--543.
https://doi.org/10.1016/j.jeconom.2016.02.005
